/*
 * Runtime chain: bug 049 (ieee802_11_if_print) -> bug 092 (beep_print).
 * PoCs from SAILOR replay_driver.c for 049 and 092.
 */
...
int main(void) {
    ...
    if (sigsetjmp(jump_buffer, 1) == 0) {
        printf("[049 -> 092] Stage 1: ieee802_11_if_print\n");
    ...
    unsigned char *bp = malloc(P_DATA_092_LEN);
    memcpy(bp, p_data_092, P_DATA_092_LEN);
    ndo2->ndo_packetp = bp;
    ndo2->ndo_snapend = bp + P_DATA_092_LEN;
    ndo2->ndo_snaplen = P_DATA_092_LEN;

    if (sigsetjmp(jump_buffer, 1) == 0) {
        printf("[049 -> 092] Stage 2: beep_print\n");
        beep_print(ndo2, bp, P_DATA_092_LEN);
    }
